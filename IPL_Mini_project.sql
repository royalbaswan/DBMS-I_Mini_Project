...
(select count(*) from ipl_bidding_details d where d.bid_status ='won' and d.bidder_id=b.bidder_id )/
(select no_of_bids from ipl_bidder_points p where p.bidder_id=b.bidder_id)*100 as win_percentage
...
select s.stadium_id, s.stadium_name, s.city, count(schedule_id) as no_of_matches
from ipl_stadium s inner join ipl_match_schedule m on s.stadium_id=m.stadium_id
...
select stadium_name 'stadium name',
...
select team_id 'Bid Team', team_name 'Team Name', count(*) 'Total Bids'
from ipl_bidding_details join ipl_team on team_id = bid_team 
group by bid_team;

-- 5.	Show the team id who won the match as per the win details.
select win_details , case 
...
select t.team_id, t.team_name, sum(MATCHES_PLAYED), sum(MATCHES_WON), sum(MATCHES_LOST)
from ipl_team t inner join ipl_team_standings s on t.team_id=s.team_id
group by t.team_id,t.team_name;

-- 7.	Display the bowlers for Mumbai Indians team.

select player_name
from ipl_player ip inner join ipl_team_players itp on ip.player_id=itp.player_id 
...
select it.team_id, it.team_name, count(player_role) count_of_all_rounders
from ipl_team it inner join ipl_team_players itp on it.team_id=itp.team_id
